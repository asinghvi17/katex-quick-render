...
export default {
  ...
  activate(state) {
    ...
    this._editorSubs = new CompositeDisposable(); // To be disposed when editor changed.
  ...
  _attachToEditor() {
    ...
    if (this._hasEditor()) {
      ...
      this._editorSubs.add(this.editor.onDidChange(() => this.update()));
      this._editorSubs.add(this.editor.onDidChangeCursorPosition(() =>
        this.update()));
      this.update();
