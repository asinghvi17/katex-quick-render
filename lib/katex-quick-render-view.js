...
export default class KatexQuickRenderView {
  ...
  constructor(serializedState) {
    ...
    this.editorSubs.add(atom.styles.addStyleSheet('', {
      sourcePath: this.getKatexCSSURL()
    }));
    this.editorSubs.add(atom.styles.addStyleSheet('', {
      sourcePath: atom.packages.getLoadedPackage('katex-quick-render').path +
                      "test/yolo.css"
    }));
  ...
  addCss() {
    ...
    link.type = "text/css";
    link.rel = "stylesheet";
    link.href = this.getKatexCSSURL();

    document.head.appendChild(link);
  }

  getKatexCSSURL() {
    return atom.packages.getLoadedPackage('katex-quick-render').path +
